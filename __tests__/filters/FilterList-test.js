...
describe('FilterList', () => {
	...
	describe('behaviour', () => {
		...
		it('calls onApply with null when space is pressed on a remove button', () => {
			const getFilterTitle = jest.fn();
			const getValues = jest.fn();
			const filters = {view: ['abc']};
			const onApply = jest.fn();
			...
			wrapper.find('[onKeyDown]').simulate('keydown', event);
			expect(event.target.closest.mock.calls).toEqual([['[data-kind="remove"]']]);
			expect(event.stopPropagation.mock.calls).toEqual([[]]);
			expect(onApply.mock.calls).toEqual([['view', null]]);
		});

		it('calls setFilter when space is pressed on a filter button', () => {
			const getFilterTitle = jest.fn();
			const getValues = jest.fn();
			const filters = {view: ['abc']};
			const setFilter = jest.fn();
			...
			useState.mockReturnValue([null, setFilter]);
			const wrapper = shallow(
				<FilterList filters={filters} excludedKeys={[]} getFilterTitle={getFilterTitle} getValues={getValues} />
			);
			wrapper.find('[onKeyDown]').simulate('keydown', event);
			expect(event.target.closest.mock.calls).toEqual([['[data-kind="remove"]'], ['[data-kind="item"]']]);
			expect(event.stopPropagation.mock.calls).toEqual([[]]);
		...
		it('does not call setFilter when other key is pressed on a filter button', () => {
			const getFilterTitle = jest.fn();
			const getValues = jest.fn();
			const filters = {view: ['abc']};
			const setFilter = jest.fn();
			...
			wrapper.find('[onKeyDown]').simulate('keydown', {keyCode: 27});
			expect(setFilter.mock.calls).toEqual([]);
		});
